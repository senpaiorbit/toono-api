// lib/formatter.js — Data normalizers used by all parsers
...
export function parseDetailsList(html = '') {
  const details = {};
  // Each <li class="rw sm"><span>Key</span><span>Value</span></li>
 ...
 * Handles both the card view and the tooltip/hover info block.
 */
export function parseCard(html = '') {
  ...
  // Poster image inside post-thumbnail
  ...
  // Year from card header meta
  const yearM = html.match(/<span[^>]*class="year"[^>]*>(\d{4})<\/span>/i);

  // Tooltip block (role="tooltip") for full detail
